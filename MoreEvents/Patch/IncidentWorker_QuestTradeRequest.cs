...
namespace MoreEvents.Patch
{
    ...
    {
        static bool Prefix(TradeRequestComp __instance, ref Caravan caravan)
        {
            for(int i = 0; i < QuestsManager.Communications.Quests.Count; i++)
            {
                Quest quest = QuestsManager.Communications.Quests[i];

                if (quest is Quest_TradeRequestComp quest2)
                {
                    WorldObject holder = __instance.parent;
                    if(quest2.SettlementBase == holder)
                    {
                        quest2.EndQuest(null, EndCondition.Success);
                    }
                }
            }
    ...
    public class IncidentWorker_QuestTradeRequest : IncidentWorker
    {
        ...
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
            ...
            Quest_TradeRequestComp quest = new Quest_TradeRequestComp(component, map.Tile);
            quest.id = QuestsManager.Communications.UniqueIdManager.GetNextQuestID();
            quest.SettlementBase = settlementBase;
            quest.TicksToPass = component.expiration;
            quest.Target = settlementBase;
            quest.Faction = settlementBase.Faction;
        ...
        private static bool TryFindRandomRequestedThingDef(Map map, out ThingDef thingDef, out int count)
        {
            ...
            Func<ThingDef, bool> globalValidator = delegate (ThingDef td)
            {
                ...
                return (td.thingSetMakerTags == null || !td.thingSetMakerTags.Contains("RewardSpecial")) ? true : false;
            };
        ...
        private static List<Thing> GenerateRewardsFor(ThingDef thingDef, int quantity, Faction faction, Map map)
        {
            ...
            parms.totalMarketValueRange = RewardValueFactorRange * RewardValueFactorFromWealthCurve.Evaluate(map.wealthWatcher.WealthTotal) * thingDef.BaseMarketValue * quantity;
